...
class Environment {
    ...
    createRenderer() {
        this.renderer = new THREE.WebGLRenderer({ antialias: true, canvas: document.querySelector('#magic') });
...
class CreateParticles {
    ...
    setup() {
        ...
        this.planeArea = new THREE.Mesh(geometry, material); this.planeArea.visible = false;
        this.createText();
    }
    ...
    onMouseDown(event) {
        ...
        const vector = new THREE.Vector3(this.mouse.x, this.mouse.y, 0.5); vector.unproject(this.camera);
        const dir = vector.sub(this.camera.position).normalize(); const distance = -this.camera.position.z / dir.z;
        this.currenPosition = this.camera.position.clone().add(dir.multiplyScalar(distance));
        this.buttom = true; this.data.ease = 0.01;
    }
    ...
    render() {
        ...
        if (intersects.length > 0) {
            ...
            for (var i = 0, l = pos.count; i < l; i++) {
                ...
                // --- HATA BURADAYDI: initZ tanımlanmamıştı ---
                const initZ = copy.getZ(i); // <-- DÜZELTİLDİ
                // ---------------------------------------------
