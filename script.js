const preload = () => {
    console.log("[KAHIN] Adım 1: Script başlatılıyor.");

    if (typeof THREE === 'undefined') {
        console.error("[KAHIN HATA] Three.js kütüphanesi yüklenemedi!");
        return;
    }
    console.log("[KAHIN] Adım 2: Three.js kütüphanesi bulundu.");
    ...
    manager.onLoad = function() { 
        console.log("[KAHIN] Adım 3: Tüm harici dosyalar başarıyla yüklendi.");
        if(typo && particle) {
    ...
    manager.onError = (url) => console.error(`[KAHIN HATA] Şu dosya yüklenemedi: ${url}`);

    console.log("[KAHIN] Adım 2.1: Font ve parçacık resmi yüklenmeye başlıyor...");
    const fontLoader = new THREE.FontLoader(manager);
    fontLoader.load('https://res.cloudinary.com/dydre7amr/raw/upload/v1612950355/font_zsd4dr.json', (font) => { typo = font; });
...
class Environment {
    ...
    render() { if (this.createParticles) this.createParticles.render(); this.renderer.render(this.scene, this.camera); }
...
class CreateParticles {
    ...
    bindEvents(){ document.addEventListener('mousemove', this.onMouseMove.bind(this)); }
    ...
    render() {
        ...
        if (intersects.length > 0) {
            const pos = this.particles.geometry.attributes.position; const copy = this.geometryCopy.attributes.position;
            const coulors = this.particles.geometry.attributes.customColor; const size = this.particles.geometry.attributes.size;
            const mx = intersects[0].point.x; const my = intersects[0].point.y;
            for (var i = 0, l = pos.count; i < l; i++) {
                const initX = copy.getX(i); const initY = copy.getY(i);
                let px = pos.getX(i); let py = pos.getY(i); let pz = pos.getZ(i);
                this.colorChange.setHSL(.5 + zigzagTime, 1, .5); coulors.setXYZ(i, this.colorChange.r, this.colorChange.g, this.colorChange.b);
                coulors.needsUpdate = true; size.setX(i, this.data.particleSize); size.needsUpdate = true;
                let dx = mx - px; let dy = my - py;
                let d = (dx * dx) + (dy * dy); const f = -this.data.area / d;
                if (this.distance(mx, my, px, py) < this.data.area) {
    ...
    createText() { 
        console.log("[KAHIN] Adım 7: createText() çalışıyor...");
        ...
        console.log("[KAHIN] Adım 8: Parçacıklar oluşturuldu. Animasyon başlamalı.");
    }
